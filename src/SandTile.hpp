...
public:
    ...
    //For GPU Packing for render
    //Buffer 0
    std::vector<BufferPixelRGBAFloat32> buffer0;
    //open gl using ints way - not working :( - need to manually compile raylib i think
    //std::vector<BufferPixelRGBAInt32> buffer0;
    ...
    SandTile(int _tile_size, IntVector _position, DebugFlags* _flags) {
        ...
        buffer0.resize(tile_size * tile_size);
    ...
    void setGrid(Particle& particle, IntVector pos) {

        int curIndex = index(pos);
        grid[curIndex] = particle;

        updateBuffers(particle, curIndex);
    }
